...
- BE STRICT. The default is is_lead=false. Set is_lead=true only when BOTH
  conditions (a) and (b) from the LEAD DEFINITION are present in the
  TARGET MESSAGE itself (not just the surrounding context).
- A request for REVIEWS / FEEDBACK / "anyone worked with X" is NOT a lead
  unless the asker also states their own need.
- A request for CONTACTS / introductions is NOT a lead unless paired with
  the asker's stated need.
- A complaint about a past provider is NOT a lead.
- Use the surrounding context to interpret SHORT target messages and to
  detect SELLER VOICE (if recent context shows the same author pitching,
  this message is unlikely to be a buyer signal).
